...
public:
  ...
  geometry_msgs::PointStamped stcCenterVelodyne;//stcCenterin as seen in the velodyne refrence frame
  ...
  void pointCloudAnalysisCallback(const sensor_msgs::PointCloud2::ConstPtr& input)
  {
    ...
    this->zPartitioner(cloudOfIntrest, bubble);
    ...
    {
      ROS_ERROR("too spooky! 0 or 1 partition(s). nonupdating stcCenters is known to cause this error, im gonna bubble/=10");
      this->bubble/=10;
      return;
    }
  ...
  //Lookup would require extrapolation into the past.
  {
    ...
    try
    {
      this->enuToVelodyneListener.waitForTransform("/enu", "/velodyne",ros::Time::now(), ros::Duration(3.0));
...
private:
  ...
  float bubble = .1;//bubble for zPartitioner 
  ...
  void zPartitioner(pcl::PointCloud<pcl::PointXYZ>::Ptr master, float bubble)
  {
    ...
    while (avgOfPartitions->size()>0)
    {
      int lowestZIdx=0;
      for(size_t j;j<avgOfPartitions->size();++j)
      {
        if (avgOfPartitions->points[j].z<avgOfPartitions->points[lowestZIdx].z)
        {
          lowestZIdx = j;
        }
      }
      cloudRefined->points.push_back(avgOfPartitions->points[lowestZIdx]);
      avgOfPartitions->points.erase(avgOfPartitions->points.begin()+lowestZIdx);
